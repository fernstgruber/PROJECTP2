...
ChannelNetworkBaseLevel = 10 m
Channel_Network_Base_Level = 50 m
...
load(file="/media/fabs/Volume/01_PAPERZEUG/paper2data/RDATA/channalnetwork_df.RData")
...
boxplot(data=sgu_df,as.formula(paste(num,"~","Abbrev.")),las=2)
  ...
  #save(sgu_df,file="/media/fabs/Volume/01_PAPERZEUG/paper2data/RDATA/channalnetwork_df.RData")
```

```{r}
sample=sgu_df[sample(1:nrow(sgu_df),5000,replace = F),]
plot(x=sample$ChannelNetworkBaseLevel,y=sample$Channel_Network_Base_Level)
summary(lm(data=sample,ChannelNetworkBaseLevel~Channel_Network_Base_Level))
```
