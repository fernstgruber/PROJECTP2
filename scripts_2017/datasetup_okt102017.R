...
sampledata <- merge(basedata,paramscale_10m,by.x="ID",by.y="ID_10m",all.x=T)
mean(unique(basedata$ID) %in% unique(paramscale_10m$ID_10m))
mean(unique(paramscale_10m$ID_10m) %in% unique(basedata$ID))
